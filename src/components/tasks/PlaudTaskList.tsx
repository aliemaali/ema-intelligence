...
export function PlaudTaskList({ initialItems,view='all' }:{ initialItems:PlaudTaskItem[]; view?:PlaudTaskView }) {
  ...
  function complete(id:string) {
    startTransition(async () => {
      try {
        ...
        setMessage('Eintrag als erledigt gespeichert und synchronisiert.')
        router.refresh()
      } catch (error) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[.16em] text-[#5CB800]">Offen · geteilt</p><h2 className="mt-1 text-2xl font-extrabold text-[#07142F]">{sectionTitle}</h2></div><span className="rounded-full bg-slate-100 px-3 py-1.5 text-sm font-extrabold">{open.length}</span></div><div className="mt-5 space-y-3">{open.length ? open.map((item) => <TaskRow key={item.id} item={item} pending={pending} onComplete={complete} />) : <p className="rounded-2xl bg-slate-50 p-5 text-sm font-semibold text-slate-500">{emptyText}</p>}</div></section>
    {completed.length > 0 && <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7"><div className="flex items-center justify-between"><h2 className="text-xl font-extrabold text-[#07142F]">Erledigt</h2><span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-extrabold">{completed.length}</span></div><div className="mt-4 space-y-3">{completed.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 p-4"><div className="min-w-0"><p className="truncate font-bold text-slate-500 line-through">{item.title}</p><p className="mt-1 text-xs text-slate-400">{item.kind === 'appointment' ? 'Termin' : 'Aufgabe'} · erledigt</p></div>{item.can_delete ? <button disabled={pending} onClick={() => remove(item.id)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border bg-white text-red-600" aria-label={`„${item.title}“ löschen`}><Trash2 className="h-5 w-5" /></button> : <span className="text-xs font-bold text-slate-400">Nur Ersteller kann löschen</span>}</div>)}</div></section>}
...
function TaskRow({ item,pending,onComplete }:{ item:PlaudTaskItem; pending:boolean; onComplete:(id:string)=>void }) {
  ...
  return <article className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#1F2A44]/8"><Icon className="h-5 w-5 text-[#1F2A44]" /></span><div className="min-w-0 flex-1"><span className="text-[10px] font-extrabold uppercase tracking-wide text-[#5CB800]">{item.kind === 'appointment' ? 'Termin' : 'Aufgabe'} · Team</span><h3 className="mt-1 font-extrabold text-[#07142F]">{item.title}</h3>{item.detail && <p className="mt-1 text-sm text-slate-500">{item.detail}</p>}{due && <p className="mt-2 flex items-center gap-1 text-xs font-semibold text-slate-400"><Clock3 className="h-3.5 w-3.5" />{due}</p>}</div></div><button disabled={pending} onClick={() => onComplete(item.id)} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#5CB800] px-4 py-3 font-extrabold text-white"><Check className="h-4 w-4" />Als erledigt markieren</button></article>
}
