...
type ProjectListSummary = { country: string; project_count: number; created_at: string }

export function CountryProjectFolders({ projects, projectLists }: { projects: any[]; projectLists: ProjectListSummary[] }) {
  const grouped = new Map<string, { count: number; pvKwp: number; listCount: number; listedProjects: number }>()
  for (const project of projects) {
    const country = String(project.location_country || 'Ohne Länderzuordnung')
    const current = grouped.get(country) ?? { count: 0, pvKwp: 0, listCount: 0, listedProjects: 0 }
    current.count += 1
