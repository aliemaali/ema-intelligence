...
export async function CountryProjectFolders({ projects }: { projects: any[]; projectLists?: unknown[] }) {
  ...
  return <section className="px-3 md:px-0">
    ...
    <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{countries.map(([country, data]) => <Link key={country} href={`/projects/country/${encodeURIComponent(country)}`} className="group rounded-[1.7rem] border border-slate-200 bg-white p-5 shadow-[0_12px_34px_rgba(15,23,42,0.06)] transition hover:-translate-y-1 hover:border-[#5CB800]/40"><div className="flex items-center justify-between"><span className="text-4xl">{flags[country] ?? '🌍'}</span><ArrowRight className="h-5 w-5 text-slate-300 transition group-hover:translate-x-1 group-hover:text-[#5CB800]" /></div><h3 className="mt-4 text-2xl font-extrabold text-[#07142F]">{country}</h3><p className="mt-2 text-sm font-bold text-slate-500">{data.count} aktive Projekte</p>{data.listCount > 0 && <><p className="mt-1 text-xs font-extrabold text-[#2F8A00]">{data.listedProjects} Projekte in {data.listCount} Projektliste{data.listCount === 1 ? '' : 'n'}</p><p className="mt-1 text-xs font-extrabold text-[#2F8A00]">{formatPowerFromKwp(data.listedKwp)} in Projektliste</p></>}<p className="mt-1 text-xs font-bold text-slate-400">{formatPowerFromKwp(data.pvKwp)} aktiv</p></Link>)}</div>
  </section>
}
