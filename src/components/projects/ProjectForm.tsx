...
export function ProjectForm({ project, partners = [], mode }: ProjectFormProps) {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(projectData?.project_image_url ?? null)
  ...
  return (
    <form action={handleSubmit} className="space-y-0">
      ...
      <input type="hidden" name="status" value={project?.status ?? 'lead'} />
      <input type="hidden" name="existing_project_image_url" value={projectData?.project_image_url ?? ''} />
        ...
        <div className="mb-6 flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar">
          {STEPS.map((item, index) => (
            <div key={item.key} className="flex shrink-0 items-center gap-1.5">
              <button type="button" onClick={() => index < stepIndex && setStep(item.key)} className={cn('flex items-center gap-1.5 text-xs font-medium', item.key === step ? 'text-[#5CB800]' : index < stepIndex ? 'text-foreground' : 'text-muted-foreground')}>
                <span className={cn('flex h-5 w-5 items-center justify-center rounded-full text-2xs font-bold', item.key === step ? 'bg-[#5CB800] text-white' : index < stepIndex ? 'bg-foreground text-background' : 'bg-muted text-muted-foreground')}>{index < stepIndex ? '✓' : index + 1}</span>
      ...
      <div className={mode === 'create' && step !== 'type' ? 'hidden' : 'mb-6 space-y-3'}>
        {mode === 'create' && <h2 className="text-base font-semibold">Projekttyp wählen</h2>}
        <div className="grid grid-cols-1 gap-2">
          {(Object.entries(PROJECT_TYPE_LABELS) as [ProjectType, string][]).map(([type, label]) => (
            <button key={type} type="button" onClick={() => setProjectType(type)} className={cn('flex items-center gap-3 rounded-lg border p-3 text-left', projectType === type ? 'border-[#5CB800] bg-[#5CB800]/5' : 'border-border bg-card')}>
              <span className="text-sm font-medium">{label}</span>{projectType === type && <span className="ml-auto text-[#5CB800]">✓</span>}
            </button>
          ))}
        </div>
      ...
      <div className={mode === 'create' && step !== 'general' ? 'hidden' : 'mb-6 space-y-4'}>
        {mode === 'create' && <h2 className="text-base font-semibold">Allgemeine Daten</h2>}
        <div><label className="form-label">Projektname *</label><input name="project_name" required defaultValue={project?.project_name} className="form-input" /></div>
        {partners.length > 0 && <div><label className="form-label">Partner</label><select name="partner_id" className="form-input" defaultValue={project?.partner_id ?? ''}><option value="">– Kein Partner –</option>{partners.map((partner) => <option key={partner.id} value={partner.id}>{partner.full_name}{partner.company ? ` · ${partner.company}` : ''}</option>)}</select></div>}
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><div><label className="form-label">Ansprechpartner</label><input name="contact_name" defaultValue={project?.contact_name ?? ''} className="form-input" /></div><div><label className="form-label">E-Mail</label><input name="contact_email" type="email" defaultValue={project?.contact_email ?? ''} className="form-input" /></div></div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div><label className="form-label">Land</label><select name="location_country" className="form-input" value={locationCountry} onChange={(event) => setLocationCountry(event.target.value)}>{countryOptions.map((country) => <option key={country} value={country}>{formatProjectCountryLabel(country)}</option>)}</select></div>
        ...
        {isGermanProjectCountry(locationCountry) && <div><label className="form-label">Bundesland</label><select name="location_state" className="form-input" defaultValue={project?.location_state ?? ''}><option value="">– wählen –</option>{GERMAN_STATES.map((state) => <option key={state} value={state}>{state}</option>)}</select></div>}
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <UnitField label="Pachtdauer" name="lease_term_years" unit="Jahre" step="1" defaultValue={projectData?.lease_term_years} />
        </div>
        <div className="rounded-2xl border border-dashed border-slate-300 p-4">
          <label className="form-label">Projektbild (optional)</label>
          {imagePreview && <img src={imagePreview} alt="Projektvorschau" className="mb-3 h-36 w-full rounded-xl object-cover" />}
          <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-slate-100 px-4 py-3 text-sm font-semibold text-[#1F2A44]"><ImagePlus className="h-4 w-4" />Bild auswählen<input name="project_image" type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) setImagePreview(URL.createObjectURL(file)) }} /></label>
          <p className="mt-2 text-xs text-muted-foreground">JPG, PNG oder WEBP, maximal 8 MB. Ohne Bild verwendet EMA ein passendes Standardmotiv.</p>
        </div>
      ...
      <div className={mode === 'create' && step !== 'technical' ? 'hidden' : 'mb-6 space-y-4'}>
        ...
        ].map(([key, label, field]) => <div key={key} className="flex items-center justify-between py-1.5"><span className="text-sm">{label}</span><DevStatusTriToggle name={key} defaultValue={project?.dev_status?.[field as keyof typeof project.dev_status]} /></div>)}</div>}
      ...
      <div className={mode === 'create' && step !== 'status' ? 'hidden' : 'mb-6 space-y-4'}>
        {mode === 'create' && <h2 className="text-base font-semibold">Status & Priorität</h2>}
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div><label className="form-label">Projektstatus</label><select name="project_stage" className="form-input" defaultValue={projectData?.project_stage ?? 'planung'}>{PROJECT_STAGE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>
      ...
      <div className={mode === 'create' && step !== 'review' ? 'hidden' : 'space-y-4'}>
        {mode === 'create' && <><h2 className="text-base font-semibold">Übersicht</h2><p className="text-sm text-muted-foreground">Typ: <strong>{PROJECT_TYPE_LABELS[projectType]}</strong> – Projektnummer wird automatisch vergeben.</p></>}
        <div className="flex gap-2">{mode === 'create' && <button type="button" onClick={() => setStep('status')} className="btn-secondary">← Zurück</button>}{mode === 'edit' && <button type="button" onClick={() => router.back()} className="btn-secondary">Abbrechen</button>}<button type="submit" disabled={pending} className="btn-primary flex-1 sm:flex-none">{pending ? <><LoadingSpinner size="sm" /> Speichern…</> : mode === 'create' ? '✓ Projekt erstellen' : '✓ Änderungen speichern'}</button></div>
      </div>
    </form>
  )
}
...
function DevStatusTriToggle({ name, defaultValue }: { name: string; defaultValue?: boolean | null }) {
  ...
  const style = value === true ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-600' : value === false ? 'bg-red-500/10 border-red-500/30 text-red-600' : 'bg-muted border-border text-muted-foreground'
  return <><input type="hidden" name={name} value={value === null ? '' : String(value)} /><button type="button" onClick={() => setValue((current) => current === null ? true : current === true ? false : null)} className={cn('flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs font-medium', style)}><span>{value === true ? '✅' : value === false ? '❌' : '⚪'}</span><span>{value === true ? 'Ja' : value === false ? 'Nein' : 'Offen'}</span></button></>
}