...
import { generateMemorandumPdf, waitForImages } from '@/lib/pdf/memorandumPdf'
...
export function PrintButton({ projectName, projectNumber }: PrintButtonProps) {
  ...
  const downloadPdf = async () => {
    ...
    try {
      await withPdfRenderMode(async () => {
        await generateMemorandumPdf(getPages(), buildFilename(projectName, projectNumber))
      })
    } catch (error) {
  ...
  const print = async () => {
    ...
    try {
      await Promise.all(getPages().map((page) => waitForImages(page)))
      await withPdfRenderMode(async () => {
        window.print()
      })
    } finally {
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .pdf-render .memorandum-page + .memorandum-page {
          break-before: page !important;
          page-break-before: always !important;
        }

        .pdf-render .memorandum-page:last-child {
      ...
      <div className="print:hidden flex flex-wrap gap-3">
        ...
        >
          ...
          {isPreparing ? 'PDF wird erstellt…' : 'Als PDF speichern'}
        </button>
