...
export function ProjectListImportPreview({ importId, initialRows }: { importId: string; initialRows: Row[] }) {
  ...
  async function createPdf() {
    try {
      ...
      const response = await fetch('/api/projektliste/pdf', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ records: chosenRows, subtitle: `${country} – ${plantType}` }) })
