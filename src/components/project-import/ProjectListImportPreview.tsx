...
const countryCodes: Record<string, string> = { Frankreich: 'FR', Deutschland: 'DE', Italien: 'IT', Spanien: 'ES', Türkei: 'TR', Niederlande: 'NL' }
...
export function ProjectListImportPreview({ importId, initialRows }: { importId: string; initialRows: Row[] }) {
  ...
  async function createPdf() {
    try {
      ...
      const response = await fetch('/api/projektliste/pdf', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ records: chosenRows, subtitle: `${localizedCountry} – ${plantType}`, language, country, countryCode: countryCodes[country] ?? 'XX' }) })
  ...
  return <form action={submit} className="card-padded rounded-[2rem] border border-[#5CB800]/30">
    ...
    <div className="mt-5 rounded-2xl border border-[#5CB800]/25 bg-[#5CB800]/8 p-4">
      ...
      <label className="mt-4 block"><span className="text-xs font-extrabold uppercase tracking-wider text-slate-500">Land</span><select value={country} onChange={(e) => { setCountry(e.target.value); setDisplayName(`EMA Projektliste ${e.target.value} ${new Date().getFullYear()}`); setPdfBlob(null) }} className="mt-2 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm font-extrabold text-[#07142F]">{Object.keys(countryCodes).map((name) => <option key={name}>{name}</option>)}<option>Sonstiges</option></select></label>
