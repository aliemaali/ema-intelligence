    ...
    <form action={submit} className="card-padded rounded-[2rem] border border-[#5CB800]/30">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div><h2 className="text-lg font-extrabold text-[#07142F]">Projektliste prüfen</h2><p className="text-sm text-slate-500">Jede ausgewählte Zeile wird als eigener Projektordner angelegt.</p></div>
        </div>
        <div className="flex gap-2 text-xs font-extrabold"><span className="rounded-full bg-[#5CB800]/10 px-3 py-1.5 text-[#2F8A00]">{selectedCount} ausgewählt</span>{warningCount > 0 && <span className="rounded-full bg-amber-50 px-3 py-1.5 text-amber-700">{warningCount} prüfen</span>}</div>
      </div>

      <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => setRows((current) => current.map((row) => ({ ...row, selected: true })))} className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-extrabold text-[#07142F]">Alle auswählen</button><button type="button" onClick={() => setRows((current) => current.map((row) => ({ ...row, selected: false })))} className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-extrabold text-[#07142F]">Alle abwählen</button></div>
      ...
      <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-2xl border border-[#5CB800]/25 bg-[#5CB800]/8 p-4"><input type="checkbox" name="confirmed" value="yes" required className="mt-1 h-5 w-5 accent-[#5CB800]" /><span className="text-sm font-bold leading-6 text-[#07142F]">Ich habe die ausgewählten Zeilen geprüft. Nur neue Projekte dürfen angelegt werden; erkannte Dubletten sollen übersprungen werden.</span></label>
      {message && <p className="mt-4 rounded-xl bg-slate-50 px-4 py-3 text-sm font-bold text-[#07142F]">{message}</p>}
      <button type="submit" disabled={selectedCount === 0 || pending} className="btn-primary mt-4 w-full justify-center py-3 disabled:opacity-50">{pending ? <Loader2 className="h-5 w-5 animate-spin" /> : <CheckCircle2 className="h-5 w-5" />} {pending ? 'Projekte werden angelegt ...' : `${selectedCount} Projekte anlegen`}</button>
    </form>
  )
}
