    ...
    <div className="min-h-[calc(100vh-8rem)] bg-gradient-to-b from-[#f7f9fc] via-white to-[#f4f8f1] pb-8 pt-[max(1rem,env(safe-area-inset-top))] md:rounded-[2rem] md:px-8 md:pt-8">
      <div className="mx-auto w-full max-w-5xl px-4 md:px-0">
        <div className="mb-5 flex w-full items-center justify-between gap-3">
          <Link
            href="/dashboard"
            className="inline-flex min-h-12 items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-extrabold text-[#07142F] shadow-sm transition hover:border-[#5CB800]/40"
          >
          ...
          <button
            ...
            className="inline-flex min-h-12 items-center gap-2 rounded-full bg-[#07142F] px-4 py-2 text-sm font-extrabold text-white shadow-sm transition active:scale-[0.98]"
          >
        ...
        <section className="overflow-hidden rounded-[1.9rem] bg-white shadow-[0_22px_70px_rgba(15,23,42,0.09)] md:rounded-[2.2rem]">
          <div className="relative overflow-hidden rounded-[1.9rem] bg-gradient-to-br from-[#07142F] via-[#091a3b] to-[#16472f] px-5 py-7 text-white md:rounded-[2.2rem] md:px-9 md:py-10">
            ...
            <div className="absolute bottom-0 right-0 h-44 w-56 opacity-20 [background-image:repeating-radial-gradient(ellipse_at_bottom_right,transparent_0,transparent_8px,rgba(255,255,255,.35)_9px,transparent_10px)]" />
            ...
            <div className="relative mt-7 flex flex-col items-start gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              ...
              <div className="relative w-full sm:w-auto">
                ...
                <select
                  ...
                  className="w-full appearance-none rounded-full border border-white/15 bg-white/10 py-3 pl-12 pr-11 text-sm font-extrabold text-white outline-none sm:min-w-[290px]"
            ...
            <div className="flex min-h-[520px] flex-col overflow-hidden rounded-[1.7rem] border border-slate-200 bg-white shadow-sm">
              ...
              <div className="flex-1 space-y-4 overflow-y-auto bg-slate-50/50 p-4 md:p-5">
                {messages.map((message) => (
                  <div key={message.id} className={`flex gap-3 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-6 shadow-sm ${message.role === 'user' ? 'rounded-br-md bg-[#5CB800] font-medium text-white' : 'rounded-bl-md border border-slate-200 bg-white text-slate-700'}`}>
                      {message.text}
                    </div>
                    {message.role === 'user' && (
              ...
              <form onSubmit={handleSubmit} className="border-t border-slate-100 bg-white p-3 md:p-4">
                <div className="flex items-end gap-2 rounded-[1.35rem] border border-slate-200 bg-slate-50 p-2 focus-within:border-[#5CB800]/60 focus-within:ring-4 focus-within:ring-[#5CB800]/10">
                  <button type="button" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-slate-500 transition hover:bg-white hover:text-[#07142F]" aria-label="Datei hinzufügen – folgt in der nächsten Ausbaustufe" title="Datei-Upload folgt">
                  ...
                  />
                  <button type="submit" disabled={!input.trim()} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#5CB800] text-white shadow-sm transition active:scale-95 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Nachricht senden">
