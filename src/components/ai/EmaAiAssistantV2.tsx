          ...
          <div className="relative overflow-hidden rounded-[1.9rem] bg-gradient-to-br from-[#07142F] via-[#091a3b] to-[#16472f] px-5 py-7 text-white md:rounded-[2.2rem] md:px-9 md:py-10">
            ...
            <div className="relative flex items-start gap-4 md:gap-6">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300 md:text-lg md:leading-8">
                  Prüft die Exposé-Daten, berechnet die Amortisation und erstellt anschließend das Investoren-PDF.
                </p>
              </div>
            </div>
            ...
            <div className="relative mt-7 flex flex-col gap-3">
              <span className="inline-flex w-fit items-center gap-2 rounded-full bg-white/10 px-4 py-3 text-sm font-extrabold text-white ring-1 ring-white/10">
                <Sparkles className="h-5 w-5 text-[#87d33b]" /> Exposé-Engine aktiv
              </span>
          ...
          {selectedProject ? (
            <div className="space-y-5 p-4 md:p-7">
              <div className="grid gap-5 md:grid-cols-[0.8fr_1.2fr]">
                <div className="rounded-[1.7rem] border border-slate-200 bg-slate-50/70 p-5">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#5CB800]">Exposé-Bereitschaft</p>
                      <p className="mt-2 text-4xl font-extrabold text-[#07142F]">{calculation?.completenessScore ?? 0}%</p>
                    </div>
                    <TrendingUp className="h-9 w-9 text-[#5CB800]" />
                  </div>
                  <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-200">
                    <div className="h-full rounded-full bg-[#5CB800] transition-all" style={{ width: `${calculation?.completenessScore ?? 0}%` }} />
                  </div>
                  <div className="mt-5 space-y-3">
                    {calculation?.required.map((item) => (
                      <div key={item.label} className="flex items-center justify-between rounded-xl bg-white px-3 py-3 shadow-sm">
                        <span className="text-sm font-bold text-[#07142F]">{item.label}</span>
                        {item.complete ? <CheckCircle2 className="h-5 w-5 text-emerald-600" /> : <AlertTriangle className="h-5 w-5 text-amber-500" />}
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  {calculation?.complete ? (
                    <div className="mt-5 rounded-2xl bg-[#07142F] p-5 text-white">
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                        <div><p className="text-slate-400">Jahresproduktion</p><p className="font-extrabold">{calculation.annualProduction.toLocaleString('de-DE', { maximumFractionDigits: 0 })} kWh</p></div>
                    ...
                    <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
                      <strong>Noch nicht bereit:</strong> Es fehlen {calculation?.missing.join(', ') || 'notwendige Werte'}.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-[1.7rem] border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex items-start gap-3">
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#5CB800]/10 text-[#378c00]"><FileText className="h-6 w-6" /></span>
                  <div className="min-w-0 flex-1">
                    <h2 className="text-lg font-extrabold text-[#07142F]">Investoren-Exposé erstellen</h2>
                    <p className="mt-1 text-sm leading-6 text-slate-500">Der PDF-Button wird erst freigegeben, wenn die vier notwendigen Werte vollständig sind.</p>
                  </div>
                </div>
                <div className="mt-5">
                  {calculation?.complete && exposeProject ? (
                    <ExposeButton project={exposeProject as any} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#07142F] px-5 py-4 text-sm font-extrabold text-white shadow-lg transition active:scale-[0.99]" />
                  ) : (
                    <button disabled className="w-full rounded-2xl bg-slate-200 px-5 py-4 text-sm font-extrabold text-slate-500">Exposé noch nicht freigegeben</button>
                  )}
                </div>
              </div>
            </div>
