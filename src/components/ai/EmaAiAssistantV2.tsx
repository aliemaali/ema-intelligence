...
export type EmaAiProject = {
  ...
  bessMw: number | null
  bessMwh: number | null
  bessDurationHours: number | null
  dataCenterGridMw: number | null
  dataCenterItMw: number | null
  landAreaSqm: number | null
  hvDistanceMeters: number | null
  fiberDistanceMeters: number | null
...
const PROJECT_KIND_DETAILS = {
  pv: { label: 'PV', metricLabel: 'PV-Leistung', icon: Sun },
  bess: { label: 'BESS', metricLabel: 'Speicher', icon: BatteryCharging },
  hybrid: { label: 'Hybrid', metricLabel: 'PV + BESS', icon: Zap },
  dataCenter: { label: 'Rechenzentrum', metricLabel: 'Netzanschlussleistung', icon: Server },
...
export function EmaAiAssistantV2({ projects }: { projects: EmaAiProject[] }) {
  ...
  const [bessMw, setBessMw] = useState('')
  const [bessMwh, setBessMwh] = useState('')
  const [bessDurationHours, setBessDurationHours] = useState('')
  ...
  const selectedKind = projectKind(selectedProject?.projectType ?? null)
  const kindDetails = PROJECT_KIND_DETAILS[selectedKind]
  const KindIcon = kindDetails.icon

  useEffect(() => {
    setPvKwp(selectedProject?.pvKwp ? String(selectedProject.pvKwp).replace('.', ',') : '')
    setBessMw(selectedProject?.bessMw ? String(selectedProject.bessMw).replace('.', ',') : '')
    setBessMwh(selectedProject?.bessMwh ? String(selectedProject.bessMwh).replace('.', ',') : '')
    const calculatedDuration = selectedProject?.bessMw && selectedProject?.bessMwh
      ? selectedProject.bessMwh / selectedProject.bessMw
      : null
    setBessDurationHours(selectedProject?.bessDurationHours
      ? String(selectedProject.bessDurationHours).replace('.', ',')
      : calculatedDuration
        ? formatGermanDecimalInput(calculatedDuration, 1)
        : '')
    ...
    try {
      const response = await fetch('/api/ema-ai/save', {
        ...
        body: JSON.stringify({
          ...
          bessMw: decimalValue(bessMw),
          bessMwh: decimalValue(bessMwh),
          bessDurationHours: decimalValue(bessDurationHours),
      ...
      if (selectedKind === 'pv' || selectedKind === 'hybrid') {
        setSpecificYield(formatGermanDecimalInput(specificYield, 2))
        setTariff(formatGermanDecimalInput(tariff, 4))
      }
      setSaveMessage('Gespeichert. EMA verwendet die Werte beim nächsten Öffnen.')
    } catch (error) {
  ...
  const calculation = useMemo(() => {
    ...
    const storageMw = decimalValue(bessMw)
    const storageMwh = decimalValue(bessMwh)
    const durationHours = decimalValue(bessDurationHours)
    const gridMw = decimalValue(selectedProject.dataCenterGridMw)
    const itMw = decimalValue(selectedProject.dataCenterItMw)
    const landSqm = decimalValue(selectedProject.landAreaSqm)
    const hvDistance = decimalValue(selectedProject.hvDistanceMeters)
    const fiberDistance = decimalValue(selectedProject.fiberDistanceMeters)
    let required: Array<{ label: string; complete: boolean }>

    if (selectedKind === 'bess') {
      required = [
        { label: 'Kaufpreis', complete: price > 0 },
        { label: 'BESS-Leistung', complete: storageMw > 0 },
        { label: 'BESS-Kapazität', complete: storageMwh > 0 },
        { label: 'Speicherdauer', complete: durationHours > 0 },
      ]
    } else if (selectedKind === 'dataCenter') {
      required = [
        { label: 'Netzanschlussleistung', complete: gridMw > 0 },
        { label: 'Grundstücksfläche', complete: landSqm > 0 },
        { label: 'HV-Distanz', complete: hvDistance > 0 },
        { label: 'Glasfaser-Distanz', complete: fiberDistance > 0 },
      ]
    } else if (selectedKind === 'hybrid') {
      required = [
        { label: 'Kaufpreis', complete: price > 0 },
      ...
      required = [{ label: 'Unterstützter Projekttyp', complete: false }]
    }
    ...
    return { kwp, price, yieldPerKwp, tariffValue, storageMw, storageMwh, durationHours, gridMw, itMw, landSqm, hvDistance, fiberDistance, annualProduction, annualRevenue, amortizationYears, required, missing, completenessScore, complete: missing.length === 0 }
  }, [selectedProject, selectedKind, pvKwp, purchasePrice, specificYield, tariff, bessMw, bessMwh, bessDurationHours])
  ...
  const exposeProject = selectedProject ? {
    ...
    bess_mw: decimalValue(bessMw) || selectedProject.bessMw || null,
    bess_mwh: decimalValue(bessMwh) || selectedProject.bessMwh || null,
    bess_duration_h: decimalValue(bessDurationHours) || selectedProject.bessDurationHours || null,
  ...
  const metricValue = (() => {
    ...
    if (selectedKind === 'bess') {
      const values = [
        decimalValue(bessMw) > 0 ? `${decimalValue(bessMw).toLocaleString('de-DE', { maximumFractionDigits: 2 })} MW` : null,
        decimalValue(bessMwh) > 0 ? `${decimalValue(bessMwh).toLocaleString('de-DE', { maximumFractionDigits: 2 })} MWh` : null,
      ].filter(Boolean)
      return values.join(' / ') || 'Noch offen'
    }
    if (selectedKind === 'dataCenter') return calculation?.gridMw ? `${calculation.gridMw.toLocaleString('de-DE', { maximumFractionDigits: 2 })} MW` : 'Noch offen'
    if (selectedKind === 'hybrid') {
      const pv = decimalValue(pvKwp) > 0 ? `${decimalValue(pvKwp).toLocaleString('de-DE', { maximumFractionDigits: 2 })} kWp` : 'PV offen'
      const storage = decimalValue(bessMwh) > 0 ? `${decimalValue(bessMwh).toLocaleString('de-DE', { maximumFractionDigits: 2 })} MWh` : 'BESS offen'
      return `${pv} / ${storage}`
    }
    if (selectedKind === 'pv') return decimalValue(pvKwp) > 0 ? `${decimalValue(pvKwp).toLocaleString('de-DE', { maximumFractionDigits: 2 })} kWp` : 'Noch offen'
    return kindDetails.label
  })()
          ...
          <div className="relative overflow-hidden rounded-[1.9rem] bg-gradient-to-br from-[#07142F] via-[#091a3b] to-[#16472f] px-5 py-7 text-white md:rounded-[2.2rem] md:px-9 md:py-10">
            ...
            <div className="relative flex items-start gap-4 md:gap-6">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300 md:text-lg md:leading-8">Erkennt PV, BESS und Rechenzentren, prüft die jeweils passenden Exposé-Daten und erstellt das Investoren-PDF.</p>
              </div>
            </div>
          ...
          {selectedProject ? (
            <div className="space-y-5 p-4 md:p-7">
              <div className="grid gap-5 md:grid-cols-[0.8fr_1.2fr]">
                ...
                <div className="rounded-[1.7rem] border border-slate-200 bg-slate-50/70 p-5">
                  <div className="flex items-start gap-3"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#5CB800]/10 text-[#378c00]">{selectedKind === 'bess' ? <BatteryCharging className="h-6 w-6" /> : selectedKind === 'dataCenter' ? <Server className="h-6 w-6" /> : <Calculator className="h-6 w-6" />}</span><div><h2 className="text-lg font-extrabold text-[#07142F]">{selectedKind === 'pv' ? 'PV-Amortisation' : selectedKind === 'bess' ? 'BESS-Kerndaten' : selectedKind === 'dataCenter' ? 'Standort-Kerndaten' : selectedKind === 'hybrid' ? 'Hybrid-Kerndaten' : 'Projektprüfung'}</h2><p className="mt-1 text-sm leading-6 text-slate-500">EMA-AI hat den Projekttyp als <strong>{kindDetails.label}</strong> erkannt und prüft nur die dafür relevanten Werte.</p></div></div>

                  {(selectedKind === 'pv' || selectedKind === 'hybrid') && <>
                    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                      <label className="block"><span className="text-xs font-bold text-slate-500">PV-Leistung kWp</span><input value={pvKwp} onChange={(event) => { setPvKwp(event.target.value); setSaveMessage('') }} inputMode="decimal" className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-[#07142F] outline-none focus:border-[#5CB800]" /></label>
                      ...
                      {selectedKind === 'hybrid' ? <label className="block"><span className="text-xs font-bold text-slate-500">BESS-Kapazität MWh</span><input value={bessMwh} onChange={(event) => { setBessMwh(event.target.value); setSaveMessage('') }} inputMode="decimal" className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-[#07142F] outline-none focus:border-[#5CB800]" /></label> : <label className="block"><span className="text-xs font-bold text-slate-500">Vergütung ct/kWh oder €/kWh</span><input value={tariff} onChange={(event) => { setTariff(event.target.value); setSaveMessage('') }} onBlur={() => setTariff(formatGermanDecimalInput(tariff, 4))} inputMode="decimal" className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-[#07142F] outline-none focus:border-[#5CB800]" /></label>}
                    </div>
                    ...
                    {selectedKind === 'pv' && calculation?.complete && <div className="mt-5 rounded-2xl bg-[#07142F] p-5 text-white"><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#87d33b]">Berechnete Amortisation</p><p className="mt-2 text-4xl font-extrabold">{calculation.amortizationYears.toLocaleString('de-DE', { maximumFractionDigits: 1 })} Jahre</p><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><p className="text-slate-400">Jahresproduktion</p><p className="font-extrabold">{calculation.annualProduction.toLocaleString('de-DE', { maximumFractionDigits: 0 })} kWh</p></div><div><p className="text-slate-400">Jahreserlös</p><p className="font-extrabold">{calculation.annualRevenue.toLocaleString('de-DE', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 })}</p></div></div></div>}
                  </>}

                  {selectedKind === 'bess' && <>
                    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                      <label className="block"><span className="text-xs font-bold text-slate-500">BESS-Leistung MW</span><input value={bessMw} onChange={(event) => { setBessMw(event.target.value); setSaveMessage('') }} inputMode="decimal" className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-[#07142F] outline-none focus:border-[#5CB800]" /></label>
                      <label className="block"><span className="text-xs font-bold text-slate-500">BESS-Kapazität MWh</span><input value={bessMwh} onChange={(event) => { setBessMwh(event.target.value); setSaveMessage('') }} inputMode="decimal" className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-[#07142F] outline-none focus:border-[#5CB800]" /></label>
                      <label className="block"><span className="text-xs font-bold text-slate-500">Speicherdauer Stunden</span><input value={bessDurationHours} onChange={(event) => { setBessDurationHours(event.target.value); setSaveMessage('') }} inputMode="decimal" className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-[#07142F] outline-none focus:border-[#5CB800]" /></label>
                      <label className="block"><span className="text-xs font-bold text-slate-500">Kaufpreis in €</span><input value={purchasePrice} onChange={(event) => { setPurchasePrice(formatGermanIntegerInput(event.target.value)); setSaveMessage('') }} inputMode="numeric" className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-[#07142F] outline-none focus:border-[#5CB800]" /></label>
                    </div>
                    <button type="button" onClick={saveValues} disabled={isSaving} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#5CB800] px-4 py-3 text-sm font-extrabold text-white shadow-sm transition active:scale-[0.99] disabled:opacity-60">{isSaving ? <Loader2 className="h-5 w-5 animate-spin" /> : <Save className="h-5 w-5" />}BESS-Werte speichern</button>
                  </>}

                  {selectedKind === 'dataCenter' && <>
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      {[['Netzanschlussleistung', calculation?.gridMw, 'MW'], ['IT-Leistung', calculation?.itMw, 'MW'], ['Grundstücksfläche', calculation?.landSqm ? calculation.landSqm / 10_000 : 0, 'ha'], ['HV-Distanz', calculation?.hvDistance, 'm'], ['Glasfaser-Distanz', calculation?.fiberDistance, 'm']].map(([label, value, unit]) => <div key={String(label)} className="rounded-xl bg-white px-4 py-3 shadow-sm"><p className="text-xs font-bold text-slate-500">{label}</p><p className="mt-1 text-lg font-extrabold text-[#07142F]">{Number(value) > 0 ? `${Number(value).toLocaleString('de-DE', { maximumFractionDigits: 2 })} ${unit}` : 'Noch offen'}</p></div>)}
                    </div>
                    <Link href={`/projects/${selectedProject.id}/site-check`} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#5CB800] px-4 py-3 text-sm font-extrabold text-white shadow-sm"><Server className="h-5 w-5" />Standortprüfung öffnen</Link>
                  </>}

                  {selectedKind === 'other' && <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">Für diesen Projekttyp gibt es noch keine eigene EMA-AI-Prüflogik.</div>}
                  ...
                  {!calculation?.complete && selectedKind !== 'other' && <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><strong>Noch nicht bereit:</strong> Es fehlen {calculation?.missing.join(', ') || 'notwendige Werte'}.</div>}
                </div>
              </div>

              <div className="rounded-[1.7rem] border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start gap-3"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#5CB800]/10 text-[#378c00]"><FileText className="h-6 w-6" /></span><div className="min-w-0 flex-1"><h2 className="text-lg font-extrabold text-[#07142F]">Investoren-Exposé {kindDetails.label}</h2><p className="mt-1 text-sm leading-6 text-slate-500">Der PDF-Button wird freigegeben, sobald die projekttypspezifischen Kerndaten vollständig sind.</p></div></div><div className="mt-5">{calculation?.complete && exposeProject ? <ExposeButton project={exposeProject as any} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#07142F] px-5 py-4 text-sm font-extrabold text-white shadow-lg transition active:scale-[0.99]" /> : <button disabled className="w-full rounded-2xl bg-slate-200 px-5 py-4 text-sm font-extrabold text-slate-500">Exposé noch nicht freigegeben</button>}</div></div>
            </div>
