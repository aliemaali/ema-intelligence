...
function createLocalAnswer(input: string, project?: EmaAiProject) {
  ...
  if (text.includes('dokument') || text.includes('pdf')) {
    return 'Der Dokumenten-Upload ist jetzt aktiv. Tippe auf „Dokument prüfen“ oder auf die Büroklammer, um eine PDF, ein Foto oder einen Screenshot zu analysieren.'
  }
...
export function EmaAiAssistantV2({ projects }: { projects: EmaAiProject[] }) {
  ...
  const documentHref = selectedProjectId
    ? `/ai/document-analyse?project=${encodeURIComponent(selectedProjectId)}`
    : '/ai/document-analyse'
          ...
          <div className="grid gap-6 p-4 md:grid-cols-[0.72fr_1.28fr] md:p-7">
            <aside>
              <div className="rounded-[1.7rem] border border-slate-200/80 bg-slate-50/70 p-4 md:p-5">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-1">
                  {QUICK_ACTIONS.map((action) => {
                    ...
                    if ('href' in action) {
                      return (
                        <Link key={action.label} href={documentHref} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#5CB800]/40 hover:shadow-md active:translate-y-0">
                          {content}
                        </Link>
                      )
                    }
              ...
              <div className="mt-4 rounded-[1.6rem] border border-emerald-200 bg-emerald-50 p-4 text-sm leading-6 text-emerald-900">
                <strong>Projektanbindung und Dokument-Upload aktiv:</strong> {projects.length} Projekt{projects.length === 1 ? '' : 'e'} aus EMA sind verfügbar.
              </div>
            </aside>
            ...
            <div className="flex min-h-[520px] flex-col overflow-hidden rounded-[1.7rem] border border-slate-200 bg-white shadow-sm">
              ...
              <form onSubmit={handleSubmit} className="border-t border-slate-100 bg-white p-3 md:p-4">
                <div className="flex items-end gap-2 rounded-[1.35rem] border border-slate-200 bg-slate-50 p-2 focus-within:border-[#5CB800]/60 focus-within:ring-4 focus-within:ring-[#5CB800]/10">
                  <Link href={documentHref} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-slate-500 transition hover:bg-white hover:text-[#07142F]" aria-label="Dokument hinzufügen" title="Dokument analysieren"><Paperclip className="h-5 w-5" /></Link>
