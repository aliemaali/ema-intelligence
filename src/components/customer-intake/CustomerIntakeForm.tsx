...
const card = 'rounded-[1.8rem] border border-slate-200 bg-white shadow-sm overflow-hidden'
...
export function CustomerIntakeForm() {
  ...
  const readPhoto = (id: number, file?: File) => {
    if (!file) return
  ...
  return <div className="page-container space-y-5 pb-28">
    ...
    <form action={createProject} className="space-y-5">
      ...
      <Section title="1 · Kundendaten"><div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="md:col-span-2"><span className="text-xs font-bold text-slate-500">Bearbeiter</span><input defaultValue="Ali Ünlüer" className={field} /></label>
      </div></Section>
      ...
      <Section title="3 · Dach"><div className="grid gap-4 md:grid-cols-2">
        {[['roofType','Dachart',['Satteldach','Flachdach','Pultdach','Sonstiges']],['covering','Dacheindeckung',['Ziegel','Blech','Bitumen','Folie','Sandwich','Sonstiges']],['orientation','Ausrichtung',['Süd','Süd-Ost','Süd-West','Ost','West','Ost-West','Nord']],['shading','Verschattung',['Keine','Gering','Mittel','Stark']]].map(([key,label,options]:any)=><label key={key}><span className="text-xs font-bold text-slate-500">{label}</span><select className={field} value={details[key] || ''} onChange={e=>set(key,e.target.value)}><option value=""></option>{options.map((x:string)=><option key={x}>{x}</option>)}</select></label>)}
      ...
      <Section title="4 · Technik"><div className="grid gap-4 md:grid-cols-2">
        {[['storage','Speicher',['Ja','Nein']],['heatPump','Wärmepumpe',['Vorhanden','Geplant','Nein']],['meterCabinet','Zählerschrank',['Modern','Alt','Umbau erforderlich']],['meter','Stromzähler',['Ferraris','Digital','Smart Meter']]].map(([key,label,options]:any)=><label key={key}><span className="text-xs font-bold text-slate-500">{label}</span><select className={field} value={details[key] || ''} onChange={e=>set(key,e.target.value)}><option value=""></option>{options.map((x:string)=><option key={x}>{x}</option>)}</select></label>)}
      </div></Section>
