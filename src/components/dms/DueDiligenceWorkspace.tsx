...
import { getDmsDocumentUrl } from '@/lib/actions/dms.actions'
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
    <section className="rounded-[1.8rem] border border-slate-200 bg-white p-4 shadow-sm md:p-6">
      ...
      {assessment && <div className="mt-6 space-y-4">
        <div className={`rounded-2xl border p-5 ${decisionClass}`}><p className="text-[10px] font-extrabold uppercase tracking-[.16em]">Investment Decision</p><p className="mt-1 text-3xl font-extrabold">{assessment.decision}</p><p className="mt-2 text-sm font-bold">Gesamtscore {assessment.overallScore}/100 · {assessment.verifiedCount} verifiziert · {assessment.openCount} offen · {assessment.criticalCount} kritisch</p>{reportDocumentId && <button type="button" onClick={() => void openDocument(reportDocumentId)} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl bg-white/70 px-4 text-sm font-extrabold shadow-sm"><Download className="h-4 w-4" /> PDF-Bericht öffnen</button>}</div>
        ...
        <div className="space-y-2">{assessment.findings.map((finding) => <details key={finding.checkId} open={finding.status === 'critical'} className="rounded-2xl border border-slate-200 p-4"><summary className="cursor-pointer text-sm font-extrabold text-[#07142F]">{finding.status === 'critical' ? '🔴' : finding.status === 'verified' ? '🟢' : '🟠'} {finding.finding}</summary><div className="mt-3 space-y-2 text-sm leading-6 text-slate-600"><p><b>Folge:</b> {finding.consequence}</p><p><b>Nächster Schritt:</b> {finding.requiredAction}</p>{finding.sources.length > 0 && <div><b>Quellen:</b>{finding.sources.map((source, index) => <button type="button" onClick={() => void openDocument(source.documentId)} key={`${source.documentId}-${index}`} className="mt-1 block w-full rounded-xl bg-slate-50 p-2 text-left text-xs hover:bg-slate-100"><b>{source.documentName}{source.page ? ` · S. ${source.page}` : ''}</b>{source.excerpt ? <span className="mt-1 block text-slate-500">{source.excerpt}</span> : null}</button>)}</div>}</div></details>)}</div>
      </div>}
    </section>

    <aside className="rounded-[1.8rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><div className="flex items-center justify-between"><h2 className="font-extrabold text-[#07142F]">Datenraum-Inhalt</h2><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-extrabold text-slate-600">{documents.length}</span></div><div className="mt-4 space-y-2">{documents.map((document) => <button type="button" key={document.id} onClick={() => void openDocument(document.id)} className="flex w-full items-center gap-3 rounded-xl border border-slate-100 p-3 text-left hover:bg-slate-50"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100"><FileSearch className="h-4 w-4 text-slate-600" /></span><span className="min-w-0 flex-1"><strong className="block truncate text-xs text-[#07142F]">{document.display_name}</strong><small className="mt-1 block truncate text-[10px] text-slate-500">{document.archive_entry_path}</small></span>{document.ai_analyzed ? <CheckCircle2 className="h-4 w-4 shrink-0 text-[#5CB800]" /> : null}</button>)}</div><p className="mt-5 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-500">Die EMA-Prüfung ist eine entscheidungsunterstützende Vorprüfung. Kritische Rechts-, Steuer- und Technikpunkte benötigen weiterhin die Freigabe qualifizierter Fachleute.</p></aside>
  </div>
}
