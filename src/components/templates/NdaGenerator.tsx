...
import { bilingualNdaFileName, buildBilingualNdaPdf, DEFAULT_NDA_PURPOSE, type LegalDocumentAssets } from '@/lib/pdf/legalDocuments'
...
export function NdaGeneratorForm({ userId, investors }: Props) {
  ...
  const buildNamedPdfFile = () => {
    const doc = buildPdf()
    const fileName = bilingualNdaFileName(form.company, form.agreementDate)
    return { doc, fileName, file: new File([doc.output('blob')], fileName, { type: 'application/pdf' }) }
  }

  const downloadNamedFile = (file: File) => {
  ...
  return <section className="dms-panel nda-generator-form rounded-[2rem] border border-white/10 p-5 shadow-2xl md:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[.12em] text-[#8eee51]">Dokumenten-Generator</p><h2 className="mt-1 text-2xl font-extrabold text-white">NDA · DE/EN</h2></div></div>
      <div className="nda-subpanel mt-5 rounded-2xl p-4"><label className="text-sm font-extrabold text-white">Investor aus Liste auswählen<select className="form-input mt-2 w-full" value={selectedInvestorId} onChange={(e) => selectInvestor(e.target.value)}><option value="">Manuell eingeben / Manual entry</option>{investors.map((investor) => <option key={investor.id} value={investor.id}>{documentInvestorLabel(investor)}</option>)}</select></label><p className="mt-2 text-xs leading-5 text-slate-300">Firma, Ansprechpartner und vollständige Geschäftsanschrift werden übernommen. Vertretungs- und Unterschriftsangaben sind optional und können später vom Vertragspartner ergänzt werden. Das gespeicherte Dokument wird automatisch dem Investor zugeordnet.</p></div>
      <div className="nda-subpanel mt-5 rounded-2xl px-4 py-3 text-sm font-bold text-slate-200">Absender / Sender: EMA Enterprise GmbH</div>
      ...
      <p className="nda-subpanel mt-3 rounded-xl px-4 py-3 text-sm font-bold text-slate-200">Speicherort: Dokumente / {NDA_DOCUMENT_FOLDER_NAME}</p>
      <p className="mt-4 text-xs leading-5 text-slate-400">Hinweis: Diese Vorlage ersetzt keine individuelle Rechtsberatung. Vor dem produktiven Einsatz sollte der Vertrag rechtlich geprüft werden.</p>
