  ...
  return (
    <>
      ...
      {isTurnkey ? (
        <>
          <div className="mt-3.5 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <KpiCard label="Kaufpreis" value={eur(calc.totalCapex)} />
            <KpiCard label="Jahresproduktion" value={`${num(calc.energyY1, 0)} kWh`} />
            <KpiCard label="Jahreserlös" value={eur(calc.revenueY1)} />
            <KpiCard
              label="Amortisation"
              value={calc.staticPayback !== null ? `${num(calc.staticPayback, 1)} Jahre` : '–'}
              accent="#5CB800"
            />
          </div>
          <div className="mt-3 rounded-xl border border-[#5CB800]/30 bg-[#eef8e6] px-3 py-3 text-xs leading-5 text-[#1F2A44]">
            <strong>Einfache Amortisation:</strong> Kaufpreis ÷ Jahreserlös. OPEX, CAPEX-Aufteilung,
            Degradation, IRR, NPV und WACC werden bei dieser Kalkulationsart nicht berücksichtigt.
          </div>
        </>
      ) : (
        <>
      ...
      <div className="mt-4 flex gap-2">
        ...
        >
          {saving ? 'Speichert…' : isTurnkey ? 'Amortisation speichern' : 'Kalkulation speichern'}
        </button>
        <button
          onClick={onNew}
