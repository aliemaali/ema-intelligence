...
export async function POST(request: NextRequest) {
  try {
    ...
    const adminRecipient = process.env.PARTNER_NOTIFICATION_EMAIL ?? 'unluer@ema-enterprise.de'
    const partnerRecipient = profile?.email || user.email
    ...
    const partnerProjectUrl = `${appUrl.replace(/\/$/, '')}/partner/submissions/${submission.id}`
    ...
    if (!adminResponse.ok) {
      console.error('Outlook admin notification failed:', adminResponse.status, await adminResponse.text())
      return NextResponse.json({ sent: false, reason: 'mail_provider_error' }, { status: 502 })
    }

    let partnerConfirmationSent = false

    if (body.event === 'created' && partnerRecipient) {
      const partnerHtml = `
        <div style="background:#f6f8fb;padding:32px 16px;font-family:Arial,sans-serif;color:#1F2A44">
          <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:20px;overflow:hidden;border:1px solid #e2e8f0">
            <div style="background:#1F2A44;padding:24px 28px;color:#ffffff">
              <div style="font-size:13px;letter-spacing:.08em;text-transform:uppercase;opacity:.75">EMA Enterprise GmbH</div>
              <h1 style="margin:8px 0 0;font-size:24px">Projekt erfolgreich eingereicht</h1>
            </div>
            <div style="padding:28px">
              <p style="margin:0 0 16px;font-size:16px;line-height:1.6">Sehr geehrte Damen und Herren,</p>
              <p style="margin:0 0 20px;font-size:16px;line-height:1.6">vielen Dank. Ihr Projekt wurde erfolgreich an EMA Enterprise übermittelt und zur Prüfung aufgenommen.</p>
              <table style="width:100%;border-collapse:collapse;font-size:15px">
                <tr><td style="padding:8px 0;color:#64748b">Projekt</td><td style="padding:8px 0;font-weight:700">${escapeHtml(submission.project_name)}</td></tr>
                <tr><td style="padding:8px 0;color:#64748b">Projektart</td><td style="padding:8px 0;font-weight:700">${escapeHtml(projectType)}</td></tr>
                <tr><td style="padding:8px 0;color:#64748b">Standort</td><td style="padding:8px 0;font-weight:700">${escapeHtml(submission.location_city)}, ${escapeHtml(submission.location_state)}</td></tr>
                <tr><td style="padding:8px 0;color:#64748b">Status</td><td style="padding:8px 0;font-weight:700">Eingereicht</td></tr>
              </table>
              <p style="margin:22px 0 0;font-size:15px;line-height:1.6;color:#475569">Den aktuellen Prüfstatus können Sie jederzeit im EMA Partnerportal einsehen.</p>
              <a href="${escapeHtml(partnerProjectUrl)}" style="display:inline-block;margin-top:22px;background:#5CB800;color:#ffffff;text-decoration:none;font-weight:700;padding:14px 22px;border-radius:12px">Projekt im Portal öffnen</a>
              <p style="margin:28px 0 0;font-size:14px;line-height:1.6;color:#64748b">Mit freundlichen Grüßen<br><strong style="color:#1F2A44">EMA Enterprise GmbH</strong></p>
            </div>
          </div>
        </div>
      `

      const partnerResponse = await sendMail({
        accessToken,
        sender,
        recipient: partnerRecipient,
        subject: `Bestätigung Ihrer Projekteinreichung: ${submission.project_name}`,
        html: partnerHtml,
      })

      if (!partnerResponse.ok) {
        console.error('Outlook partner confirmation failed:', partnerResponse.status, await partnerResponse.text())
      } else {
        partnerConfirmationSent = true
      }
    }

    return NextResponse.json({ sent: true, partnerConfirmationSent })
  } catch (error) {
