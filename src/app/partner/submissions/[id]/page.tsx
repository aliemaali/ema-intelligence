  ...
  return (
    <main className="min-h-screen bg-[#F7F9FC] text-[#1F2A44]">
      <div className="mx-auto w-full max-w-3xl px-4 pb-10 pt-[max(1rem,env(safe-area-inset-top))] sm:px-6">
        ...
        <section className="mt-6 rounded-[1.9rem] bg-white p-6 shadow-sm sm:p-8">
        ...
        <section className="mt-5 rounded-[1.75rem] bg-white p-5 shadow-sm sm:p-7">
        ...
        {submission.partner_response && submission.status !== 'rueckfrage' && (
          <section className="mt-5 rounded-[1.75rem] bg-white p-5 shadow-sm sm:p-7"><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[#2F8A00]">Deine letzte Antwort</p><p className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{submission.partner_response}</p></section>
        )}
      </div>
    </main>
  )
