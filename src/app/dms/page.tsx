...
export default async function DmsPage() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_5%_18%,rgba(92,184,0,.12),transparent_24%),radial-gradient(circle_at_96%_8%,rgba(41,91,220,.16),transparent_28%),linear-gradient(145deg,#edf3f7_0%,#f8fafc_52%,#eef4fb_100%)] px-3 pb-10 pt-[calc(env(safe-area-inset-top)+1rem)] md:px-7 md:py-7">
      <div className="mx-auto max-w-[1380px]">
        <header className="mb-5 overflow-hidden rounded-[2rem] border border-white/10 bg-[#06162f] p-5 text-white shadow-[0_25px_70px_rgba(1,10,27,.26)] md:p-7">
          ...
          <div className="mt-6 grid grid-cols-3 gap-2 md:max-w-2xl md:gap-3">
            <div className="rounded-2xl border border-white/10 bg-white/[.045] p-3"><Files className="h-4 w-4 text-[#8eee51]" /><strong className="mt-2 block text-xl md:text-2xl">{documents.length}</strong><span className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Dokumente</span></div>
            <div className="rounded-2xl border border-white/10 bg-white/[.045] p-3"><FileArchive className="h-4 w-4 text-[#8eee51]" /><strong className="mt-2 block text-xl md:text-2xl">{dataRooms.length}</strong><span className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Datenräume</span></div>
            <div className="rounded-2xl border border-white/10 bg-white/[.045] p-3"><Database className="h-4 w-4 text-[#8eee51]" /><strong className="mt-2 block text-xl md:text-2xl">1</strong><span className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Zentrale</span></div>
          </div>
        </header>

        <section className="mb-5 rounded-[1.7rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><p className="mb-3 text-[10px] font-extrabold uppercase tracking-[.16em] text-[#4d930e]">Dokumente erstellen</p><div className="flex flex-wrap gap-3"><NdaGenerator userId={user.id} investors={documentInvestors} /><AdditionalDocumentGenerators userId={user.id} folders={(foldersResult.data ?? []) as any} investors={documentInvestors} /></div></section>
