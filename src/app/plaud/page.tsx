...
export default function PlaudPage() {
  ...
  return <main className="mx-auto w-full max-w-[1180px] space-y-4 px-4 pb-28 pt-[max(1rem,env(safe-area-inset-top))] md:space-y-6 md:px-6 md:py-8">
    ...
    <div className="grid grid-cols-3 gap-2 print:hidden"><Stat icon={Mic2} label="Aufnahmen" value={noteArchived ? '0' : '1'} /><Stat icon={CalendarDays} label="Offene Termine" value={String(counts.termine)} /><Stat icon={CheckSquare} label="Offene Aufgaben" value={String(counts.aufgaben)} /></div>
    ...
    {!noteArchived ? <section className="rounded-[1.75rem] border bg-white p-5 shadow-sm print:hidden">
...
function Stat({ icon:Icon,label,value }:{ icon:typeof Mic2; label:string; value:string }) {
  return <div className="rounded-2xl border bg-white p-3 shadow-sm"><Icon className="h-5 w-5" /><p className="mt-3 text-[10px] font-extrabold uppercase text-slate-500">{label}</p><p className="mt-1 text-xl font-extrabold">{value}</p></div>
}
