...
import {
  Archive, ArrowLeft, CalendarDays, Check, CheckSquare, Clock3, ExternalLink, Eye, FileText,
  Download, Languages, Link2, Loader2, Mic2, Pencil, RefreshCw, Save, Share2,
  ShieldCheck, Unlink, X,
} from 'lucide-react'
...
type PlaudNote = {
  ...
  translation:{done:boolean;completed:number;total:number}
}
...
export default function PlaudPage() {
  ...
  const [translationRunningIds, setTranslationRunningIds] = useState<string[]>([])
  const translationRunningRef = useRef(new Set<string>())
  const translationPausedRef = useRef(new Set<string>())

  const continueTranslation = useCallback(async (noteId:string, manual=false) => {
    if (translationRunningRef.current.has(noteId)) return
    if (!manual && translationPausedRef.current.has(noteId)) return
    if (manual) translationPausedRef.current.delete(noteId)
    translationRunningRef.current.add(noteId)
    setTranslationRunningIds((current) => current.includes(noteId) ? current : [...current,noteId])
    setErrorMessage('')
    try {
      for (let step=0;step<100;step+=1) {
        const response=await fetch('/api/plaud/translate',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({noteId}),
        })
        const data=await response.json().catch(()=>({}))
        if(!response.ok)throw new Error(data.error||'Die deutsche Übersetzung konnte nicht fortgesetzt werden.')
        const translation={done:Boolean(data.done),completed:Number(data.completed||0),total:Number(data.total||0)}
        setNotes((current)=>current.map((note)=>note.id===noteId?{...note,translation}:note))
        if(translation.done)break
      }
    } catch(error) {
      translationPausedRef.current.add(noteId)
      setErrorMessage(`${error instanceof Error?error.message:'Die deutsche Übersetzung wurde unterbrochen.'} Der gespeicherte Fortschritt bleibt erhalten.`)
    } finally {
      translationRunningRef.current.delete(noteId)
      setTranslationRunningIds((current)=>current.filter((id)=>id!==noteId))
    }
  },[])
  ...
  useEffect(() => {
    notes.forEach((note) => {
      if (!note.translation.done) void continueTranslation(note.id)
    })
  }, [continueTranslation, notes])
  ...
  async function decideRecording(fileId:string, accept:boolean) {
    ...
    try {
      ...
      setMessage(accept
        ? `„${data.title || 'PLAUD-Meeting'}“ wurde übernommen${data.translation?.done ? '.' : '; das deutsche Transkript wird jetzt abschnittsweise erstellt.'}${data.suggestionCount ? ` ${data.suggestionCount} Aufgaben oder Termine warten auf deine Freigabe.` : ''}`
...
function PlaudPreview({note,onClose}:{note:PlaudNote;onClose:()=>void}){const[language,setLanguage]=useState<'de'|'original'>('de');const[loading,setLoading]=useState(true);const[shareError,setShareError]=useState('');const canShowOriginal=Boolean(note.summary_original||note.source_language);const pdfUrl=`/api/plaud/notes/${encodeURIComponent(note.id)}/pdf?language=${language}`;useEffect(()=>{const previous=document.body.style.overflow;document.body.style.overflow='hidden';return()=>{document.body.style.overflow=previous}},[]);useEffect(()=>setLoading(true),[language]);async function sharePdf(){setShareError('');try{const response=await fetch(pdfUrl,{cache:'no-store'});if(!response.ok)throw new Error('PDF konnte nicht geladen werden.');const blob=await response.blob();const file=new File([blob],`EMA-PLAUD-${language==='de'?'DE':'Original'}.pdf`,{type:'application/pdf'});if(navigator.share&&navigator.canShare?.({files:[file]})){await navigator.share({title:note.title,files:[file]});return}const opened=window.open(pdfUrl,'_blank','noopener,noreferrer');if(!opened)window.location.assign(pdfUrl)}catch(error){if(error instanceof DOMException&&error.name==='AbortError')return;setShareError(error instanceof Error?error.message:'PDF konnte nicht geteilt werden.')}}return <div className="fixed inset-0 z-50 flex h-[100dvh] w-screen max-w-full flex-col overflow-hidden bg-[#111827]"><div className="shrink-0 bg-white px-3 pb-3 pt-[max(.75rem,env(safe-area-inset-top))] shadow-lg"><div className="grid grid-cols-3 gap-2"><button onClick={onClose} className="rounded-xl border py-3 text-sm font-extrabold text-[#1F2A44]"><X className="mr-1 inline h-4 w-4"/>Schließen</button><button onClick={()=>void sharePdf()} className="rounded-xl border py-3 text-sm font-extrabold text-[#1F2A44]"><Share2 className="mr-1 inline h-4 w-4"/>Teilen</button><a href={`${pdfUrl}&download=1`} className="rounded-xl bg-[#5CB800] py-3 text-center text-sm font-extrabold text-white"><Download className="mr-1 inline h-4 w-4"/>Sichern</a></div>{canShowOriginal&&<div className="mt-2 grid grid-cols-2 rounded-xl bg-slate-100 p-1"><button onClick={()=>setLanguage('de')} className={`rounded-lg py-2 text-sm font-extrabold ${language==='de'?'bg-white text-[#07142F] shadow-sm':'text-slate-500'}`}>Deutsch</button><button onClick={()=>setLanguage('original')} className={`rounded-lg py-2 text-sm font-extrabold ${language==='original'?'bg-white text-[#07142F] shadow-sm':'text-slate-500'}`}>Original</button></div>}<a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="mt-2 flex items-center justify-center rounded-xl border border-slate-200 py-2.5 text-sm font-extrabold text-[#1F2A44]"><ExternalLink className="mr-2 h-4 w-4"/>Separat im iPhone öffnen</a>{shareError&&<p role="alert" className="mt-2 text-center text-xs font-bold text-red-600">{shareError}</p>}</div><div className="relative min-h-0 flex-1 overflow-hidden">{loading&&<div className="absolute inset-0 z-10 flex items-center justify-center bg-[#111827]"><div className="rounded-2xl bg-white px-5 py-4 font-bold text-[#1F2A44]"><Loader2 className="mr-2 inline animate-spin"/>PDF wird erstellt</div></div>}<iframe key={pdfUrl} src={pdfUrl} onLoad={()=>setLoading(false)} title={`PLAUD PDF ${language==='de'?'Deutsch':'Original'}`} className="h-full w-full border-0 bg-slate-200"/></div></div>}
