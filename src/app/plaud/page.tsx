...
export default function PlaudPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/plaud/state', { cache:'no-store' })
      ...
      .catch((error) => {
        ...
        setMessage(error instanceof Error ? error.message : 'PLAUD-Daten konnten nicht geladen werden.')
      })
      .finally(() => {
  ...
  return <main className="mx-auto w-full max-w-[1180px] space-y-4 px-4 pb-28 pt-[max(1rem,env(safe-area-inset-top))] md:space-y-6 md:px-6 md:py-8">
    ...
    <div className="grid grid-cols-3 gap-2 print:hidden" aria-busy={stateLoading}><Stat href={noteArchived ? '/archive/plaud' : '#plaud-note'} icon={Mic2} label="Aufnahmen" value={stateLoading ? '–' : noteArchived ? '0' : '1'} /><Stat href="/tasks?view=appointments" icon={CalendarDays} label="Offene Termine" value={stateLoading ? '–' : String(counts.termine)} /><Stat href="/tasks?view=tasks" icon={CheckSquare} label="Offene Aufgaben" value={stateLoading ? '–' : String(counts.aufgaben)} /></div>

    <section className="rounded-[1.75rem] border bg-white p-5 shadow-sm print:hidden" aria-busy={stateLoading}><p className="text-[11px] font-extrabold uppercase tracking-[.15em] text-[#5CB800]">EMA Vorschläge</p><h2 className="mt-1 text-xl font-extrabold text-[#07142F]">{stateLoading ? 'Daten werden geladen' : stateLoadFailed ? 'Daten nicht verfügbar' : suggestions.length ? `${suggestions.length} Elemente prüfen` : 'Alles geprüft'}</h2>{stateLoading ? <LoadingCard /> : <div className="mt-5 space-y-3">{suggestions.map((item) => <Item key={item.id} item={item} disabled={pending} onAct={act} />)}</div>}{!stateLoading && !stateLoadFailed && !suggestions.length && <Link href="/tasks" className="mt-4 inline-flex w-full items-center justify-center rounded-xl border border-slate-200 px-4 py-3 font-extrabold text-[#1F2A44]">Gespeicherte Aufgaben und Termine öffnen</Link>}</section>

    {stateLoading ? <section className="rounded-[1.75rem] border bg-white p-5 shadow-sm print:hidden" aria-label="PLAUD-Notiz wird geladen" aria-busy="true"><p className="text-[11px] font-extrabold uppercase tracking-[.15em] text-[#5CB800]">PLAUD Notiz</p><LoadingCard /></section> : !noteArchived ? <section id="plaud-note" className="scroll-mt-6 rounded-[1.75rem] border bg-white p-5 shadow-sm print:hidden">
...
function LoadingCard() {
  return <div className="mt-5 animate-pulse rounded-2xl border p-4" role="status"><span className="sr-only">Daten werden geladen</span><div className="h-4 w-24 rounded bg-slate-200" /><div className="mt-4 h-5 w-4/5 rounded bg-slate-200" /><div className="mt-3 h-4 w-1/2 rounded bg-slate-100" /></div>
}
