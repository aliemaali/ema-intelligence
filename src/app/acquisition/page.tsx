...
import SwipeDeleteLeadCard from '@/components/acquisition/SwipeDeleteLeadCard'
...
export default async function AcquisitionPage({ searchParams }: { searchParams: { error?: string; deleted?: string } }) {
  const supabase = await createClient()
  const db = supabase as any
  const [{ data, error }, { count: approvalCount }] = await Promise.all([
    db.from('acquisition_leads').select('id, acquisition_type, company_name, city, contact_name, email, score, status, next_action, next_action_at, estimated_potential_kwp').is('deleted_at', null).order('created_at', { ascending: false }).limit(50),
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F9] px-4 py-6 md:px-8 md:py-8"><div className="mx-auto max-w-7xl space-y-6">
      ...
      {searchParams.deleted && <div className="rounded-xl border border-green-200 bg-green-50 p-4 text-sm text-green-800">Lead wurde gelöscht. Offene Entwürfe und Follow-ups wurden gestoppt.</div>}
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 px-5 py-4"><h2 className="font-semibold text-[#132060]">Aktuelle Leads</h2><p className="mt-1 text-xs text-slate-400">Auf dem Handy nach links wischen, um einen Lead zu löschen</p></div>
