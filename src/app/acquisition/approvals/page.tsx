...
import { ArrowLeft, Check, MailCheck, Save, Send, X } from 'lucide-react'
import { createClient } from '@/lib/supabase/server'
import { approveAcquisitionEmail, rejectAcquisitionEmail } from '@/lib/actions/acquisition.actions'
import { updateAcquisitionEmailDraft } from '@/lib/actions/email-draft-edit.actions'
...
const EDITABLE_STATUSES = ['draft', 'ready_for_approval', 'approved', 'failed']

export default async function AcquisitionApprovalsPage({
  searchParams,
}: {
  searchParams: { created?: string; bulk?: string; sent?: string; saved?: string; error?: string }
}) {
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F9] px-4 py-6 pb-44 md:px-8 md:py-8 md:pb-8">
      <div className="mx-auto max-w-5xl space-y-6">
        <div>
          ...
          <div className="mt-5 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">Betreff und Nachricht können vor der Freigabe manuell bearbeitet werden.</p>
            </div>
          </div>
        </div>
        ...
        {searchParams.saved && <div className="rounded-xl border border-green-200 bg-green-50 p-4 text-sm text-green-800">Der Entwurf wurde gespeichert und wartet erneut auf deine Freigabe.</div>}
        ...
        {searchParams.error === 'draft' && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">Der Entwurf konnte nicht gespeichert werden. Betreff und Nachricht dürfen nicht leer sein.</div>}
        ...
        {searchParams.error === 'status' && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">Diese E-Mail kann in ihrem aktuellen Status nicht bearbeitet oder versendet werden.</div>}
        ...
        {error ? (
          ...
          <div className="space-y-4">
            {emails.map((email) => {
              const editable = EDITABLE_STATUSES.includes(email.status)
                ...
                <article key={email.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                  <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-2 text-xs text-slate-500">An: {email.recipient_email}</p>
                    </div>
                  ...
                  {editable ? (
                    <form action={updateAcquisitionEmailDraft} className="space-y-5 px-5 py-5">
                      <input type="hidden" name="email_id" value={email.id} />
                      <label className="block space-y-2">
                        <span className="text-sm font-semibold text-[#132060]">Betreff</span>
                        <input
                          name="subject"
                          defaultValue={email.subject}
                          required
                          maxLength={300}
                          className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-[#132060] outline-none transition focus:border-[#5CB800] focus:ring-2 focus:ring-[#5CB800]/15"
                        />
                      </label>
                      <label className="block space-y-2">
                        <span className="text-sm font-semibold text-[#132060]">Nachricht</span>
                        <textarea
                          name="body"
                          ...
                          maxLength={20000}
                          rows={16}
                          className="w-full resize-y rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm leading-7 text-slate-700 outline-none transition focus:border-[#5CB800] focus:ring-2 focus:ring-[#5CB800]/15"
                        />
                      </label>
                      {['approved', 'failed'].includes(email.status) && (
                        <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-800">Nach einer Änderung muss die E-Mail erneut freigegeben werden. Sie kann nicht mit einem alten Freigabestatus versendet werden.</p>
                      )}
                      <button className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#132060] bg-white px-4 py-3 text-sm font-semibold text-[#132060] shadow-sm hover:bg-[#EEF2F7] sm:w-auto">
                        <Save className="h-4 w-4" /> Entwurf speichern
                      </button>
                    </form>
                  ) : (
                    <div className="px-5 py-5">
                      <h2 className="font-semibold text-[#132060]">{email.subject}</h2>
                      <div className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-700">{email.body}</div>
                    </div>
                  )}
