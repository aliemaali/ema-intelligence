...
export default async function DokumentePage() {
  ...
  const [documents, assignmentsResult, partnersResult, investorsResult, projectsResult] = await Promise.all([
  ...
  const partners = (partnersResult.data ?? []).map((item: any) => ({
    id: item.id,
    label: item.company || item.full_name || 'Partner',
    subtitle: [item.full_name, item.category].filter(Boolean).join(' · '),
  }))
  const investors = (investorsResult.data ?? []).map((item: any) => ({
    id: item.id,
    label: item.company_name || item.company || item.full_name || 'Investor',
    subtitle: item.contact_person || item.full_name || item.email,
  }))
  const projects = (projectsResult.data ?? []).map((item: any) => ({
    id: item.id,
    label: item.project_name || item.project_number || 'Projekt',
    subtitle: [item.project_number, item.location_city].filter(Boolean).join(' · '),
  }))
