...
import { ArrowLeft, Bell, Building2, CalendarDays, Camera, Save } from 'lucide-react'
...
function PhotoField({ name, title }: { name: string; title: string }) {
  return <label className="block rounded-2xl border border-slate-200 bg-[#FBFCFE] p-3">
    <span className="mb-2 block text-sm font-extrabold text-[#07142F]">{title}</span>
    <span className="flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-[#5CB800]/35 bg-white px-4 text-center text-sm font-bold text-slate-500 hover:border-[#5CB800] hover:bg-[#F6FCEB]">
      <Camera className="mb-2 h-6 w-6 text-[#3D9700]" /> Foto aufnehmen oder auswählen
...
export default function CustomerIntakePage() {
  return <div className="w-full max-w-full overflow-x-hidden pb-28">
    ...
    <main className="mx-auto max-w-[1480px] space-y-5 px-4 pt-7 md:px-8 md:pt-8">
      <Link href="/dashboard" className="inline-flex items-center gap-3 rounded-full border border-slate-200 bg-white px-5 py-3 text-base font-extrabold text-[#07142F] shadow-sm"><ArrowLeft className="h-5 w-5" /> Zurück zum Dashboard</Link>
      ...
      <section className="rounded-[2rem] bg-gradient-to-br from-[#07142F] via-[#10245A] to-[#16472f] px-6 py-8 text-white shadow-lg md:px-9 md:py-10">
        <div className="flex items-center justify-between gap-5"><div><p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[#87d33b]">EMA Intelligence</p><h1 className="mt-2 text-4xl font-extrabold leading-tight tracking-tight md:text-5xl">Kundenaufnahme PV</h1><p className="mt-3 text-base text-slate-300">Direkt in EMA Intelligence erfassen und speichern.</p></div><Building2 className="h-14 w-14 shrink-0 text-[#87d33b]" /></div>
      </section>
      ...
      <form action={submitCustomerIntake} encType="multipart/form-data" className="space-y-5">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
          <div className="space-y-5">
            <section className={card}><h2 className="text-2xl font-extrabold text-[#07142F]">1 · Kundendaten</h2><div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <section className={card}><h2 className="text-2xl font-extrabold text-[#07142F]">2 · Objekt & Dach</h2><div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <section className={card}><h2 className="text-2xl font-extrabold text-[#07142F]">3 · Technik & Projektdaten</h2><div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <aside className="xl:sticky xl:top-6 xl:self-start"><section className={card}><div className="flex items-center gap-3"><Camera className="h-6 w-6 text-[#3D9700]" /><div><h2 className="text-2xl font-extrabold text-[#07142F]">Fotos</h2><p className="text-sm text-slate-500">Vor Ort aufnehmen und frei benennen.</p></div></div><div className="mt-5 space-y-4">
            <PhotoField name="photo_meter_cabinet" title="Zählerschrank" /><PhotoField name="photo_roof" title="Dachfläche" /><PhotoField name="photo_inverter" title="Wechselrichter" /><PhotoField name="photo_building" title="Gebäudeansicht" /><PhotoField name="photo_other" title="Weiteres Foto" />
          </div><p className="mt-4 rounded-xl bg-amber-50 px-4 py-3 text-xs font-semibold leading-relaxed text-amber-800">Die Fotoauswahl ist eingebaut. Die dauerhafte Speicherung der Bilddateien in Supabase Storage wird separat verbunden.</p></section></aside>
        </div>
