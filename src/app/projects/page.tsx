...
export default async function ProjectsPage({ searchParams }: ProjectsPageProps) {
  ...
  return <div className="w-full max-w-full space-y-6 overflow-x-hidden pb-28 md:mx-auto md:max-w-[1480px] md:space-y-7">
    ...
    <section className="px-3 md:px-0"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[.2em] text-[#5CB800]">{showAll ? 'Gesamtansicht' : 'Schnellsuche'}</p><h2 className="mt-2 text-3xl font-extrabold text-[#07142F]">{showAll ? 'Alle Projekte' : 'Projekt direkt finden'}</h2></div>{!showAll && <Link href="/projects?view=all" className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-extrabold text-[#07142F]">Alle Projekte anzeigen <ArrowRight className="h-4 w-4" /></Link>}</div>
      <form className="mt-5 flex items-center gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm" action="/projects"><input type="hidden" name="view" value="all" /><Search className="ml-2 h-5 w-5 shrink-0 text-slate-400" /><input name="q" defaultValue={searchParams.q ?? ''} placeholder="Projektname, Nummer oder Standort suchen" className="min-h-11 min-w-0 flex-1 bg-transparent px-2 text-sm text-[#07142F] outline-none placeholder:text-slate-400" /><button className="min-h-11 rounded-xl bg-[#07142F] px-4 text-sm font-extrabold text-white">Suchen</button></form>

      {showAll && <><div className="mt-4 flex gap-2 overflow-x-auto pb-1 no-scrollbar">{types.map((type) => { const params = new URLSearchParams({ view: 'all' }); if (type.value) params.set('type', type.value); if (searchParams.q) params.set('q', searchParams.q); const active = (searchParams.type ?? '') === type.value; return <Link key={type.value} href={`/projects?${params.toString()}`} className={`shrink-0 rounded-full px-4 py-2.5 text-sm font-extrabold ${active ? 'bg-[#5CB800] text-white' : 'bg-white text-slate-600 ring-1 ring-slate-200'}`}>{type.label}</Link> })}</div>
