...
export type DdSource = { documentId: string; documentName: string; page: number | null; excerpt?: string }
...
export function buildDdAssessment(profile: DdProjectProfile, findings: DdFinding[]): DdAssessment {
  ...
  const lensScores = Object.fromEntries((['engineering', 'investor', 'legal'] as DdReviewLens[]).map((lens) => {
    const lensFindings = complete.filter((finding) => finding.lens === lens)
    const score = lensFindings.length ? Math.round(lensFindings.reduce((sum, finding) => sum + scoreForStatus[finding.status], 0) / lensFindings.length) : 0
    return [lens, score]
  })) as Record<DdReviewLens, number>

  const hardGateFailures = checks.filter((check) => check.hardGate && byId.get(check.id)?.status === 'critical').map((check) => check.id)
  const hardGateOpen = checks.filter((check) => check.hardGate && byId.get(check.id)?.status !== 'verified' && byId.get(check.id)?.status !== 'critical').map((check) => check.id)
  const overallScore = Math.round((lensScores.engineering + lensScores.investor + lensScores.legal) / 3)
  const verifiedCount = complete.filter((finding) => finding.status === 'verified').length
  ...
  else if (complete.some((finding) => finding.status === 'open') || overallScore < 80) decision = 'CONDITIONAL GO'
  else decision = 'GO'
  ...
  return {
    ...
    hardGateFailures,
    hardGateOpen,
    criticalCount: complete.filter((finding) => finding.status === 'critical').length,
    openCount: complete.filter((finding) => finding.status === 'open').length,
    verifiedCount,
  }
}
